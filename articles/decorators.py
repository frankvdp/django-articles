...
def logtime(func):
    ...
    def wrapped(*args, **kwargs):
        ...
        if func.__class__.__name__ == 'function':
            ...
            executing = '%s.%s.%s' % (
                func.__module__, func.__class__.__name__, func.__name__)
        else:

            executing = str(func)

        log.debug(
            'Logging execution time for %s with args: %s; kwargs: %s' %
...
def once_per_instance(func):
    ...
    def wrapped(self, *args, **kwargs):
        ...
        if name in self.__run_once_methods:
            ...
            log.debug(
                'Method %s has already been called for %s... not '
                'calling again.' % (name, self))
